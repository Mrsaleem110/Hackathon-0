...
# Import Claude reasoning engine
try:
...
class VaultOrchestrator:
    ...
    def create_plan_for_task(self, file_path: Path, content: str, action_type: str):
        """Create a plan file for the task using AI provider if available"""
        plan_filename = f"PLAN_{file_path.stem}.md"
        plan_path = self.plans_path / plan_filename

        # Try to use AI reasoning engine first
        if REASONING_ENGINE_AVAILABLE:
            try:
                engine = ReasoningEngine(vault_path=str(self.vault_path))
                task = {
                    'filename': file_path.name,
                    'content': content
                }
                plan_content = engine.generate_plan_with_ai(task)
...
ai_provider: {engine.provider if engine.provider else 'fallback'}
---
...
---
*Generated by Reasoning Engine with {engine.provider.upper() if engine.provider else 'Fallback'} AI*
*{datetime.now().isoformat()}*
"""
                    try:
                        plan_path.write_text(full_plan, encoding='utf-8')
                        logger.info(f"Created AI plan: {plan_path.name}")
                        return
                    except Exception as e:
                        logger.error(f"Error writing AI plan: {e}")
            except Exception as e:
                logger.error(f"Error using AI provider: {e}")
...
"""
        ...
        try:
            ...
            logger.info(f"Created basic plan: {plan_path.name}")
        except Exception as e:
